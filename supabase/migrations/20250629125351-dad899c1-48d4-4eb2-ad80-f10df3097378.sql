...
-- Add session_duration_minutes column to groups table
ALTER TABLE public.groups ADD COLUMN IF NOT EXISTS session_duration_minutes integer NOT NULL DEFAULT 60;

-- Add a check constraint to ensure positive duration
ALTER TABLE public.groups ADD CONSTRAINT groups_session_duration_check 
CHECK (session_duration_minutes > 0 AND session_duration_minutes <= 480);

-- Update the create_group_subscription function to use the session duration from groups table
...
BEGIN
  ...
  FOREACH current_student_uuid IN ARRAY p_student_ids
  LOOP
    -- Verify student exists and belongs to the school
    SELECT * INTO student_record FROM public.students WHERE students.id = current_student_uuid;
    
    IF student_record.id IS NULL THEN
      RETURN json_build_object('success', false, 'message', 'Student not found: ' || current_student_uuid);
    END IF;
    
    IF student_record.school_id != p_current_school_id THEN
      RETURN json_build_object('success', false, 'message', 'Student does not belong to your school: ' || current_student_uuid);
    END IF;
    ...
    VALUES (p_group_id, current_student_uuid, p_start_date)
    ...
    WHILE sessions_created < group_record.session_count LOOP
      ...
      INSERT INTO public.lesson_sessions (
        ...
        p_group_id,
        session_datetime,
        group_record.session_duration_minutes, -- Use the session duration from the group
        'scheduled',
        'paid',
        session_cost,
