...
-- Step 1: Clean up existing duplicate sessions for the specific student
WITH duplicate_sessions AS (
  SELECT 
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY 
        CASE 
          WHEN notes LIKE 'Auto-generated from subscription (session:%/%' THEN 1
...
-- Step 3: Create a proper unique constraint on exact datetime
CREATE UNIQUE INDEX idx_lesson_sessions_unique_student_datetime 
ON public.lesson_sessions (student_id, scheduled_date);

-- Step 4: Add index_in_sub column to track session sequence
ALTER TABLE public.lesson_sessions 
ADD COLUMN IF NOT EXISTS index_in_sub integer;

-- Step 5: Final version of add_student_subscription function with single session creation
...
BEGIN
  ...
  -- Process the first schedule item only
  ...
  IF schedule_item IS NOT NULL THEN
    ...
    -- **SINGLE SESSION CREATION LOOP - THIS IS THE ONLY PLACE SESSIONS ARE CREATED**
