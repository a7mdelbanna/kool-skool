...
-- Completely rewrite the session generation logic to prevent duplicates
...
BEGIN
  ...
  -- CRITICAL: Clean up any existing sessions for this subscription immediately
  ...
  -- Process ONLY the first schedule item to avoid any confusion
  ...
  IF schedule_item IS NOT NULL THEN
    ...
    CASE schedule_item ->> 'day'
      ...
      ELSE target_weekday := 0; -- Default to Monday if unknown
    END CASE;
    ...
    -- Always move to the NEXT occurrence of the target weekday
    -- If days_diff is 0 (same weekday), we still want the next week
    IF days_diff = 0 THEN
      days_diff := 7;
    END IF;
    ...
    -- Generate sessions using a single atomic operation
    -- Create exactly p_session_count sessions, no more, no less
    ...
    LOOP
      ...
      -- Insert the session with proper indexing
  ...
  -- Log the number of sessions created for debugging
...
-- Clean up any existing duplicate sessions one final time
DELETE FROM public.lesson_sessions 
WHERE id IN (
  SELECT DISTINCT ls1.id
  FROM public.lesson_sessions ls1
  INNER JOIN public.lesson_sessions ls2 ON (
    ls1.subscription_id = ls2.subscription_id 
    AND ls1.student_id = ls2.student_id 
    AND ls1.scheduled_date = ls2.scheduled_date
    AND ls1.id != ls2.id
  )
  WHERE (ls1.notes IS NULL OR ls1.notes NOT LIKE '%Auto-generated from subscription (index:%')
    AND (ls2.notes IS NOT NULL AND ls2.notes LIKE '%Auto-generated from subscription (index:%')
);
