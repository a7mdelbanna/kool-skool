...
BEGIN
  -- Validate current user (using explicit table alias)
  ...
  -- Validate student (using explicit table alias)
  ...
  IF schedule_item IS NOT NULL THEN
    ...
    LOOP
      ...
      -- Check if session already exists (using explicit column references)
      SELECT EXISTS(
  ...
  -- Return the created subscription (using explicit table alias)
  RETURN QUERY
  SELECT 
    sub.id,
    sub.student_id,
