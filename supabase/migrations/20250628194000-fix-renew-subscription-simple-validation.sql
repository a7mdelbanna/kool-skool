...
-- Fix the renew_subscription function with simplified validation matching SubscriptionsTab approach
...
DECLARE
  ...
  v_result jsonb;
BEGIN
  -- First, get the subscription record directly (same as SubscriptionsTab)
  SELECT s.*
  INTO v_current_sub
  ...
  -- Then, verify the student belongs to the correct school (same validation pattern as other functions)
  SELECT st.*
