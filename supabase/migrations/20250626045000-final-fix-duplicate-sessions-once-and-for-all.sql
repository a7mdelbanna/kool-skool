...
-- FINAL FIX: Replace ALL previous versions of add_student_subscription with ONE definitive version
-- This completely removes any possibility of duplicates by ensuring only ONE function exists

-- Drop the function completely first to ensure no conflicts
...
-- Create THE definitive version (only one that should exist)
...
BEGIN
  ...
  -- CRITICAL: Delete ANY existing sessions for this student to prevent conflicts
  ...
  IF schedule_item IS NOT NULL THEN
    ...
    -- Create sessions - EXACTLY the number requested, no more, no less
    FOR i IN 1..p_session_count
    LOOP
      session_datetime := (first_session_date + ((i - 1) * 7))::DATE + session_time::TIME;
      
      -- Insert session with proper tracking
      INSERT INTO public.lesson_sessions (
        ...
        'paid',
        session_cost,
        'FINAL-VERSION: Auto-generated from subscription (session:' || i || '/' || p_session_count || ')',
        i
      );
  ...
  -- Log the exact number created
  RAISE NOTICE 'FINAL VERSION: Created exactly % sessions for subscription %', created_sessions_count, new_subscription_id;
...
-- Clean up any orphaned sessions that might have been created by old versions
DELETE FROM public.lesson_sessions 
WHERE notes IS NULL 
OR (notes NOT LIKE 'FINAL-VERSION:%' AND created_at > NOW() - INTERVAL '1 hour');

-- Verify the cleanup
SELECT COUNT(*) as remaining_lesson_sessions FROM public.lesson_sessions;
