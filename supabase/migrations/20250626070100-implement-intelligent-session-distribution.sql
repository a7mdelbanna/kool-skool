...
-- Update the add_student_subscription function with intelligent session distribution
...
BEGIN
  ...
  WHILE sessions_created < p_session_count LOOP
    ...
    FOR i IN 1..array_length(sorted_schedule, 1) LOOP
      ...
      session_datetime := (current_date + days_to_add)::DATE + session_time::TIME;
      
      -- Only create session if it's on or after start date and we haven't reached session limit
      IF (current_date + days_to_add) >= p_start_date AND sessions_created < p_session_count THEN
        ...
        current_date := (current_date + days_to_add + 1)::DATE;
    ...
    IF sessions_created = 0 AND current_date > p_start_date + INTERVAL '1 year' THEN
