...
DECLARE
  ...
  current_student_id UUID; -- Renamed to avoid ambiguity
...
BEGIN
  ...
  FOREACH current_student_id IN ARRAY p_student_ids -- Use renamed variable
  LOOP
    -- Verify student exists and belongs to the school
    SELECT * INTO student_record FROM public.students WHERE students.id = current_student_id;
    
    IF student_record.id IS NULL THEN
      RETURN json_build_object('success', false, 'message', 'Student not found: ' || current_student_id);
    END IF;
    
    IF student_record.school_id != p_current_school_id THEN
      RETURN json_build_object('success', false, 'message', 'Student does not belong to your school: ' || current_student_id);
    END IF;
    ...
    INSERT INTO public.subscriptions (
      ...
      current_student_id, -- Use renamed variable
      p_group_id,
      'group',
    ...
    VALUES (p_group_id, current_student_id, p_start_date) -- Use renamed variable
    ...
    WHILE sessions_created < group_record.session_count LOOP
      ...
      -- Insert the session with explicit column qualification
      INSERT INTO public.lesson_sessions (
        ...
        new_subscription_id,
        current_student_id, -- Use renamed variable explicitly
        p_group_id,
        session_datetime,
