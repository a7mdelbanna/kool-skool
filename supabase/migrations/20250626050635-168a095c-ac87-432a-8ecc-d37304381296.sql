...
-- Fix the ambiguous column reference error in add_student_subscription function
-- The issue is that student_id appears in multiple tables and needs proper qualification
...
BEGIN
  ...
  -- FIX: Properly qualify the student_id column reference
  ...
  IF schedule_item IS NOT NULL THEN
    ...
    LOOP
      ...
      INSERT INTO public.lesson_sessions (
        ...
        'paid',
        session_cost,
        'FIXED-VERSION: Auto-generated from subscription (session:' || i || '/' || p_session_count || ')',
        i
      );
  ...
  RAISE NOTICE 'FIXED VERSION: Created exactly % sessions for subscription %', created_sessions_count, new_subscription_id;
