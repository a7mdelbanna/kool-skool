...
-- Fix the session generation logic in add_student_subscription function
...
BEGIN
  ...
  LOOP
    ...
    -- Check if we need to move to next week based on time
    IF days_diff = 0 THEN
      -- For simplicity, if it's the same day, we'll use it
      -- In a real scenario, you'd compare current time with session_time
      first_session_date := p_start_date;
    ELSE
