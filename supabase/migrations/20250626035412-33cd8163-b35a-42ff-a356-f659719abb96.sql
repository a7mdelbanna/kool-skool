...
BEGIN
  ...
  DELETE FROM public.lesson_sessions WHERE lesson_sessions.subscription_id = new_subscription_id;
  ...
  IF schedule_item IS NOT NULL THEN
    ...
    LOOP
      ...
      -- Check if session already exists at this exact datetime (with explicit table prefix)
      SELECT COUNT(*) INTO existing_count
      FROM public.lesson_sessions ls
      WHERE ls.student_id = p_student_id 
        AND ls.scheduled_date = session_datetime;
  ...
  -- Return the created subscription (with explicit table prefix)
  RETURN QUERY
  SELECT 
    sub.id,
    sub.student_id,
