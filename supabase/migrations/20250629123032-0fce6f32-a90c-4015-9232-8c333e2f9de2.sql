...
-- Fix all ambiguous column references in create_group_subscription function
...
BEGIN
  ...
  LOOP
    ...
    SELECT * INTO student_record FROM public.students WHERE students.id = student_id;
    
    IF student_record.id IS NULL THEN
      RETURN json_build_object('success', false, 'message', 'Student not found: ' || student_id);
    END IF;
    
    IF student_record.school_id != p_current_school_id THEN
      RETURN json_build_object('success', false, 'message', 'Student does not belong to your school: ' || student_id);
    END IF;
    ...
    WHILE sessions_created < group_record.session_count LOOP
      ...
      -- Insert the session with fully qualified column names
      INSERT INTO public.lesson_sessions (
        ...
        new_subscription_id,
        student_id, -- This is the local variable, not a column reference
        p_group_id,
        session_datetime,
