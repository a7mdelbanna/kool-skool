...
-- Clean up duplicate sessions and fix the session generation logic
-- First, let's remove duplicate sessions that don't have the proper index notation
...
WHERE id IN (
  ...
  WHERE (ls1.notes IS NULL OR ls1.notes NOT LIKE '%index:%')
    AND (ls2.notes IS NOT NULL AND ls2.notes LIKE '%index:%')
);

-- Now update the function to prevent future duplicates
...
BEGIN
  ...
  LOOP
    ...
    -- Delete any existing sessions for this subscription to prevent duplicates
    DELETE FROM public.lesson_sessions 
    ...
    LOOP
      ...
      -- Insert the session (clean slate, no duplicates possible)
