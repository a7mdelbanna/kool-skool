...
exports.setUserClaims = functions.https.onCall(async (data: any, context: any) => {
...
// Deactivate user
exports.deactivateUser = functions.https.onCall(async (data: any, context: any) => {
...
// Reactivate user
exports.reactivateUser = functions.https.onCall(async (data: any, context: any) => {
  // Verify caller is admin
  if (!context.auth || !['admin', 'superadmin'].includes(context.auth.token.role)) {
    throw new functions.https.HttpsError(
      'permission-denied',
...
exports.generateLessonSessions = functions.https.onCall(async (data: any, context: any) => {
  // Verify caller is staff
  if (!context.auth || !['admin', 'teacher'].includes(context.auth.token.role)) {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Only staff can generate sessions'
    );
  }
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      error.message || 'Failed to generate sessions'
    );
  }
});
...
exports.migrateFromSupabase = functions.https.onCall(async (data: any, context: any) => {
  // Only superadmin can trigger migration
  if (!context.auth || context.auth.token.role !== 'superadmin') {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Only superadmins can trigger migration'
    );
  }
...
exports.updatePaymentStatuses = functions.pubsub
  ...
  .onRun(async () => {
    ...
    overdueSessionsSnapshot.forEach((doc: any) => {
