...
const SubscriptionRenewalWidget: React.FC = () => {
    ...
    queryFn: async () => {
      ...
      for (const student of activeStudents) {
        try {
          ...
          // Check if subscription is expiring in the next 7 days or already expired
          const endDate = new Date(mostRecent.end_date || mostRecent.endDate);
          const today = new Date();
          const weekFromNow = addDays(today, 7);

          if (endDate <= weekFromNow && !mostRecent.is_renewed) {
  ...
  const urgentRenewals = expiringSubscriptions?.filter(sub => getDaysRemaining(sub.end_date) <= 3) || [];
  const upcomingRenewals = expiringSubscriptions?.filter(sub => getDaysRemaining(sub.end_date) > 3) || [];
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        ...
        <CardDescription>
          Students needing subscription renewal
        </CardDescription>
      </CardHeader>
      <CardContent>
        <ScrollArea className="h-[400px] pr-4">
          {expiringSubscriptions && expiringSubscriptions.length > 0 ? (
            <div className="space-y-3">
              {/* Urgent Renewals (expired or expiring in 3 days) */}
              {urgentRenewals.length > 0 && (
                <div className="space-y-3">
              ...
              {/* Upcoming Renewals (expiring in 4-7 days) */}
